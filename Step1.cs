...
// Step 1 of the exercise. This is only called once. Step 5 is used after the first iteration as step 1. (Due to the different instruction).
...
public class Step1 : MonoBehaviour {
    ...
    //initially planned on changing the material to add a glow effect - this is only available in legacy editions of unity.
    ...
    void Start() {
        ...
        using (StreamWriter sw = new StreamWriter(m_path + "/Step1_T6.txt", true))
        {
    ...
    void Update()
    {
        ...
        angle = Vector3.Angle(pos4 - pos4end, pos4 - pos3); //smaller of the two angles is used
        //Debug.Log(angle);
        ...
        //pos - now no longer in final data readings as positional movement is not possible with the glove
        Vector3 currentPositionBone3 = bone3.transform.position;
        Vector3 difference;

        if (currentPositionBone3 != lastPositionBone3)
        {
            difference = (currentPositionBone3 - lastPositionBone3);
            using (StreamWriter sw = new StreamWriter(m_path + "/Step1Tracking.txt", true))
            {
                sw.Write(difference);
                sw.Write("\r\n");
            }
        }
        lastPositionBone3 = currentPositionBone3;

        //All unused sprites deactivated
        ...
        //if (angle < 145 && angle2 < 150 && angle3 < 140 && angle4 < 135 && angle5 < 145 && angle6 < 145 && angle7 < 120 && angle8 < 140 && angle9 < 120) -- OLD METHOD
        ...
        {
            //Debug.Log("Step one complete - fingers and thumb inwards.");

            // stop timer and write time to file
            using (StreamWriter sw = new StreamWriter(m_path + "/time1_T6.txt", true))
            {
    ...
    void FanglePrint()
    {
        //print callibration angles for step 1 to file
        using (StreamWriter sw = new StreamWriter(m_path + "/Step1Fangle_T6.txt", true))
        {
